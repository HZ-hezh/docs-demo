...
const previousSong = () => {
  console.log('=== 点击上一首 ===')
  console.log('当前索引:', currentIndex.value, '播放列表长度:', playlist.value.length)
  console.log('当前歌曲:', currentSong.value ? currentSong.value.name : '无')
  
  // 检查播放列表是否为空
  if (playlist.value.length === 0) {
    console.error('播放列表为空，无法切换')
    return
  }
  ...
  console.log('目标歌曲:', playlist.value[prevIndex] ? playlist.value[prevIndex].name : '无效索引')
  
  // 边界检查
  if (prevIndex < 0 || prevIndex >= playlist.value.length) {
    console.error('计算的索引超出范围:', prevIndex, '有效范围: 0 -', playlist.value.length - 1)
    return
  }
  
  currentIndex.value = prevIndex
  console.log('索引已更新为:', currentIndex.value)
  
  if (audioRef.value && currentSong.value) {
    console.log('开始切换到上一首:', currentSong.value.name)
    ...
    // 设置新的音源
    audioRef.value.src = currentSong.value.src
    console.log('新音源已设置:', currentSong.value.src)
