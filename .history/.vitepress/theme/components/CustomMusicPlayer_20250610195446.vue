...
const processAudioSrc = (src) => {
  ...
  if (isProduction && src.startsWith('/api/music')) {
    ...
    if (isHTTPS) {
      // 如果当前网站使用HTTPS，但音频服务器只支持HTTP，这里可能会有混合内容问题
      console.warn('Mixed content warning: HTTPS site trying to load HTTP audio:', src)
      // 可以尝试使用HTTPS版本，或者提示用户
      return src.replace('/api/music', 'https://47.113.102.160:9000/docs-demo/music')
    } else {
      // HTTP环境下直接使用HTTP
