...
export function set_sidebar(dirPath) {
  ...
  return generateSidebarItems(fullPath, dirPath, false)
}
 ...
 */
function generateSidebarItems(dir, basePath, isSubMenu = false) {
  const items = []
  ...
  try {
    ...
    files.forEach(file => {
      ...
      if (file.isDirectory()) {
        // 处理子目录
        const subItems = generateSidebarItems(filePath, `${basePath}/${file.name}`, true)
