...
<style>
.VPTeamMembers .VPTeamMembersItem {
  cursor: pointer;
}
.VPTeamMembers .VPTeamMembersItem:hover {
...
<VPTeamPage>
  ...
  <VPTeamMembers size="small" :members="members" @click="handleMemberClick" />
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // 使用更长的延迟确保VitePress组件完全渲染
  setTimeout(() => {
    console.log('Looking for team members');
    ...
    for (let selector of selectors) {
      memberCards = document.querySelectorAll(selector);
      console.log(`Found ${memberCards.length} elements with selector: ${selector}`);
      if (memberCards.length > 0) break;
    }
    ...
    if (memberCards.length === 0) {
      ...
      document.addEventListener('click', function(e) {
        // 检查是否点击的是团队成员相关元素
        if (e.target.closest('[class*="Team"]') || e.target.closest('[class*="team"]')) {
          console.log('Team member clicked via delegation');
      ...
      memberCards.forEach((card, index) => {
        console.log(`Adding click event to card ${index}`);
